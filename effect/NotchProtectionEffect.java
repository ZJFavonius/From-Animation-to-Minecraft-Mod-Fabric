...
public class NotchProtectionEffect extends StatusEffect {
    public NotchProtectionEffect(StatusEffectCategory category, int color) {
        super(category, color);
    }

    public boolean applyUpdateEffect(LivingEntity player, int amplifier) {
        if (player instanceof PlayerEntity) {
            player.addStatusEffect(new StatusEffectInstance(StatusEffects.RESISTANCE, 200, 7, false, false));
