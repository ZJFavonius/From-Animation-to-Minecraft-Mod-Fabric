...
public class StrawHatItem extends Item {
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        tooltip.add(Text.literal("Mugiwara").formatted(Formatting.GOLD));
        tooltip.add(Text.literal("").formatted(Formatting.GOLD));

        if (net.minecraft.client.gui.screen.Screen.hasShiftDown()) {

            tooltip.add(Text.literal("Gol D. Roger").formatted(Formatting.GOLD));
            tooltip.add(Text.literal("Red-Haired Shanks").formatted(Formatting.RED));
            tooltip.add(Text.literal("Monkey D. Luffy").formatted(Formatting.WHITE));
            tooltip.add(Text.literal("").formatted(Formatting.GOLD));
            tooltip.add(Text.literal("\uD83D\uDC80NE P\uD83E\uDDCDEC€").formatted(Formatting.DARK_BLUE));
            tooltip.add(Text.literal("").formatted(Formatting.WHITE));
            tooltip.add(Text.literal("The Straw Hat is a pivotal symbol in One Piece").formatted(Formatting.DARK_AQUA));
            tooltip.add(Text.literal("Luffy promised to return it to Shanks once he").formatted(Formatting.DARK_AQUA));
            tooltip.add(Text.literal("becomes a great pirate, and it is the symbol").formatted(Formatting.DARK_AQUA));
            tooltip.add(Text.literal("of his crew, the Straw Hat Pirates.").formatted(Formatting.DARK_AQUA));
            tooltip.add(Text.literal("").formatted(Formatting.WHITE));
        } else {
