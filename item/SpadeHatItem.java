...
public class SpadeHatItem extends Item {

    public SpadeHatItem(Settings settings) {
        super(settings);
        }
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        tooltip.add(Text.literal("Spade Pirates Jolly Roger Hat").formatted(Formatting.RED));
        tooltip.add(Text.literal("").formatted(Formatting.GOLD));

        if (net.minecraft.client.gui.screen.Screen.hasShiftDown()) {

            tooltip.add(Text.literal("Portgas D. Ace").formatted(Formatting.RED));
            tooltip.add(Text.literal("").formatted(Formatting.GOLD));
            tooltip.add(Text.literal("\uD83D\uDC80NE P\uD83E\uDDCDEC€").formatted(Formatting.DARK_BLUE));
            tooltip.add(Text.literal("").formatted(Formatting.WHITE));
            tooltip.add(Text.literal("This is signature piece of headwear that").formatted(Formatting.DARK_RED));
            tooltip.add(Text.literal("features a smiling and frowning \"smiley\"").formatted(Formatting.DARK_RED));
            tooltip.add(Text.literal("design on the front, a Spade symbol, and").formatted(Formatting.DARK_RED));
            tooltip.add(Text.literal("flames protruding from the top, which").formatted(Formatting.DARK_RED));
            tooltip.add(Text.literal("represent his fire-based abilities.").formatted(Formatting.DARK_RED));
            tooltip.add(Text.literal("").formatted(Formatting.WHITE));
        } else {
