...
public class HolyLlamaHammerItem1 extends AxeItem {
    public HolyLlamaHammerItem1(ToolMaterials toolMaterials, Settings settings) {
    ...
    public void summonbackup(World world, PlayerEntity player) {
        if (!world.isClient) {
            ...
            this.blessingoftheLlama(player.getWorld(), player);
        }
    ...
    public void appendTooltip(ItemStack stack, Item.TooltipContext context, List<Text> tooltip, TooltipType type) {
        ...
        tooltip.add(Text.literal("Jessica doesn't hold back").formatted(Formatting.GOLD));
        tooltip.add(Text.literal("").formatted(Formatting.GOLD));
        tooltip.add(Text.literal("From the Annoying Villagers by MrFudgeMonkeyz Studios").formatted(Formatting.GREEN));
        tooltip.add(Text.literal("").formatted(Formatting.GOLD));
        tooltip.add(Text.literal("Jessica stands as a testament to the llamas' guardians, a symbol of").formatted(Formatting.GOLD));
        tooltip.add(Text.literal("hope and salvation for all who dare to dream of a brighter future.").formatted(Formatting.GOLD));
        tooltip.add(Text.literal("").formatted(Formatting.GOLD));
