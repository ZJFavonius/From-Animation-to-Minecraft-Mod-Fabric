...
public class EnderAegisShieldFB extends ShieldItem {
    private final Multimap<EntityAttribute, EntityAttributeModifier> attributeModifiers;
    private final boolean isEnderSightActive;

    public EnderAegisShieldFB(Settings settings) {
        super(settings);
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (user.isSneaking()) {
            if (itemStack.isOf(ModItems.ENDER_AEGIS_SHIELDFB)) {
                user.setStackInHand(hand, new ItemStack(ModItems.ENDER_AEGIS_SHIELD));
            }
    ...
    public void handlePlayerInput(PlayerEntity player) {
        if (player.isUsingItem() && player.getActiveItem().getItem() instanceof EnderAegisShieldFB) {
            ItemStack stack = player.getActiveItem();
            Item var4 = stack.getItem();
            if (var4 instanceof EnderAegisShieldFB) {
                EnderAegisShieldFB shield = (EnderAegisShieldFB)var4;
