...
public class GrowingStaffItem extends SwordItem {
    public GrowingStaffItem(ToolMaterials toolMaterials, Settings settings) {
    ...
    public float getBonusAttackDamage(Entity target, float baseAttackDamage, DamageSource damageSource) {
        return super.getBonusAttackDamage(target, 10, damageSource);
    }
    ...
    private void addEnchantment(ItemStack stack, RegistryEntryLookup<Enchantment> lookup) {
        lookup.getOptional(ModEnchantments.COMMITTED).ifPresent(entry -> stack.addEnchantment(entry, 3));
    }

    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        tooltip.add(Text.literal("[UNIQUE]").formatted(Formatting.GOLD));
        tooltip.add(Text.literal("♦Increased damage to wounded mobs").formatted(Formatting.GOLD));
        ...
        if (net.minecraft.client.gui.screen.Screen.hasShiftDown()) {
            ...
            tooltip.add(Text.literal("A staff that grows and shifts").formatted(Formatting.YELLOW));
            tooltip.add(Text.literal("as it attacks, the Growing Staff").formatted(Formatting.YELLOW));
            tooltip.add(Text.literal("is unpredictable and powerful.").formatted(Formatting.YELLOW));
        } else {
