...
public class GraveBaneItem extends SwordItem {
    public GraveBaneItem(ToolMaterials toolMaterials, Settings settings) {
    ...
    private void addEnchantment(ItemStack stack, RegistryEntryLookup<Enchantment> lookup) {
        lookup.getOptional(ModEnchantments.SMITING).ifPresent(entry -> stack.addEnchantment(entry, 3));
    }
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        ...
        tooltip.add(Text.literal("◆Extra damage to undead").formatted(Formatting.GOLD));
        ...
        if (net.minecraft.client.gui.screen.Screen.hasShiftDown()) {
            ...
            tooltip.add(Text.literal("A relic from ages of darkness;").formatted(Formatting.GOLD));
            tooltip.add(Text.literal("this glaive radiates potent magical").formatted(Formatting.GOLD));
            tooltip.add(Text.literal("energy to ward off the undead.").formatted(Formatting.GOLD));
        } else {
        ...
        tooltip.add(Text.literal("Ṭਠ").formatted(Formatting.LIGHT_PURPLE));
